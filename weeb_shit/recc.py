...
class AnimeRecommender:
   ...
   def calculate_similarity_matrix(cls, anime_dataset):
       try:
           # Make sure we're working with strings, not categorical
           features = ['Genres', 'Type', 'Studios', 'Source', 'Name']
           feature_data = {}
           for feature in features:
               # Convert categorical back to string if necessary
               if str(anime_dataset[feature].dtype) == 'category':
                   feature_data[feature] = anime_dataset[feature].astype(str).fillna('')
               else:
                   feature_data[feature] = anime_dataset[feature].fillna('')
           
           # Combine features efficiently
           combined_features = (
               feature_data['Genres'].astype(str) + ' ' + 
               feature_data['Type'].astype(str) + ' ' + 
               feature_data['Studios'].astype(str) + ' ' + 
               feature_data['Source'].astype(str) + ' ' + 
               feature_data['Name'].astype(str)
           )
       ...
       if not self._initialized:
           try:
               ...
               try:
                   ...
                   # Calculate similarity matrix first
                   ...
                   # Now convert string columns to category to save memory
